...
class TableLogger:
    __slots__ = ("start_time", "current_values", "data_frame", 
                "has_timestamp", "smart_dash", "logging_finished", 
                "start_timestamp", "output_file")
    ...
    def update_timestamp(self):
        ...
        if not self.has_timestamp:
            self.current_values["Timestamp"] = (time.perf_counter() 
                                              - self.start_time,)

    def parse_args(self) -> argparse.Namespace:
        """ Parse command-line arguments
        """
        parser = argparse.ArgumentParser(description="Log the SmartDashboard "
                                                     "entries on a "
                                                     "NetworkTables server.")

        parser.add_argument("IP", metavar="i", type=str, nargs=1,
                            help="The IP or team number of the "
                            "NetworkTables server.")
        
        parser.add_argument("directory", metavar="D", type=str, 
                            nargs="?", default="./",
                            help="The directory where the csv file should"
                            " be outputted. Defaults to the current"
                            " open directory.")
        
        parser.add_argument("check_time", metavar="T", type=int, 
                            nargs="?", default=50,
                            help="The number of milliseconds"
                            " between each log update."
                            " Defaults to 50 milliseconds.")
        
        parser.add_argument("flush_time", metavar="F", type=float, 
                            nargs="?", default=3,
                            help="The number of seconds"
                            " between each log flush."
    ...
    def initialize_logger(self, ip: str, directory: str):
        """ Initialize the NetworkTables client 
            and update the initial current_values.
        """
        if len(ip) <= 4:
        ...
        self.output_file = (output_directory + "SDlog_" + 
                           time.strftime("%Y-%m-%d_%H:%M:%S") + ".csv")

    def append_to_df(self) -> None:
        """ Append all current values to the log dataframe 
            and print the status to the user.
        """
        self.update_timestamp()
        curr_df = pd.DataFrame.from_dict(self.current_values, orient="columns")
        print("\n"*20
            + ("Now logging SmartDashboard values."
               "Press SPACE to stop logging.\n"
               "Press F to manually flush the log to a csv. "
               "(occurs automatically every 3 seconds).\n")
            +f"Data on current frame:\n{curr_df}")
        self.data_frame = pd.concat([self.data_frame, curr_df], 
                                    ignore_index=True)
    ...
    async def log(self, frame_time: float, flush_time: float) -> None:
        ...
        async def check_keyboard() -> None:
            """ Check keyboard inputs every 20 ms.
                Exit if the user presses space, and 
                flush the log if the user presses f.
            """
            flush_cooldown = 0.0
